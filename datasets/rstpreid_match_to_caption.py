        ...
        if training:
            ...
            for anno in annos:
                ...
                for caption, match_score in zip(captions, match_scores):
                    ...
                    # pdb.set_trace()
                    # 拼接新的caption，添加相关性描述
                    new_caption = f"{caption} In summary, the degree of relevance to the image is {match_score_float}"
                    dataset.append((pid, image_id, img_path, new_caption, match_score_float))
                image_id += 1
