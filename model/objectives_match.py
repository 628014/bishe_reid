...
# 新增匹配分数回归损失函数
def compute_match(predicted_scores, target_scores):
    """
    计算匹配分数的回归损失
    Args:
        predicted_scores: 模型预测的匹配分数，shape: [batch_size]
        target_scores: 真实的匹配分数，shape: [batch_size]
    
    Returns:
        loss: MSE损失值
    """
    mse_loss = nn.MSELoss()
    return mse_loss(predicted_scores, target_scores)
