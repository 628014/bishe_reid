...
# 扩展ImageTextMLMDataset类，支持返回match_score字段
class ImageTextMLMDatasetMatch(ImageTextMLMDataset):
    def __getitem__(self, index):
        # 调用父类的__getitem__方法
        ret = super().__getitem__(index)
        # 原始数据格式：(pid, image_id, img_path, caption, match_score)
        _, _, _, _, match_score = self.dataset[index]
        # 添加match_score字段
        ret['match_score'] = torch.tensor(match_score, dtype=torch.float32)
        return ret
...
def do_train(start_epoch, args, model, train_loader, evaluator0, evaluator1, evaluator2, optimizer,
    ...
    meters = {
        ...
        "match_loss": AverageMeter(),  # 新增匹配分数回归损失
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        ...
        for n_iter, batch in enumerate(train_loader):
            ...
            ret = model(batch)
            # 查看这个ret是什么
            ret = {key: values.mean() for key, values in ret.items() if key != 'match_pred'}
            ...
            meters['match_loss'].update(ret.get('match_loss', 0), batch_size)  # 更新匹配分数回归损失
